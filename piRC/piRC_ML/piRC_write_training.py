...
def main():
    ...
    while True:
        try:
            ...
            s,p = statMotors()
            print(' S={0:.0f}, P={1:.0f}, L={2:.2f}, R={3:.2f}, C={4:.2f}, B={5:.2f}, X={6:.3f}, Y={7:.3f}, Z={8:.3f}'.format(s,p,l,r,c,b,x,y,z))
            with open(filename, "a") as sum_file:
                sum_file.write('{0:.0f}\t{1:.0f}\t{2:.2f}\t{3:.2f}\t{4:.2f}\t{5:.2f}\t{6:.3f}\t{7:.3f}\t{8:.3f}\n'.format(s,p,l,r,c,b,x,y,z))
        except:
            fullStop()
            GPIO.cleanup()
            return
